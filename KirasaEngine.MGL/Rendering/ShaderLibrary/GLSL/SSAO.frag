...
layout(std140, binding = 0) uniform SSAOConstants
{
    vec4 Params0; // x = tan(fovY/2), y = aspect ratio, z = radius, w = power
    vec4 Params1; // x = sample count, y = bias, z/w unused
};

layout(binding = 0) uniform sampler2D NormalDepthTexture;

layout(location = 0) out float oOcclusion;

// Fibonacci/"sunflower" spiral over a hemisphere: well-distributed samples with no lookup texture and no
// per-sample storage, just a closed-form function of the sample index. `seed` rotates the spiral per pixel
// to turn the (very fast, low sample-count) residual banding into less objectionable noise.
...
void main()
{
    ...
    // Nothing was drawn here (cleared background) — leave fully unoccluded.
    if (depth <= 0.0)
    {
        oOcclusion = 1.0;
        return;
    ...
    for (int i = 0; i < sampleCount; i++)
    {
        ...
        vec2 sampleUV = vec2(samplePos.x / sampleNegZ / (tanHalfFovY * aspect), samplePos.y / sampleNegZ / tanHalfFovY) * 0.5 + 0.5;
